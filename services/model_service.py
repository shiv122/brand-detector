...
class ModelService:
    ...
    def _load_available_weights(self):
        ...
        for weight_file in weights_dir.glob("*.pt"):
            try:
                ...
                self.config.available_weights.append(
                    WeightInfo(
                ...
                print(
                    f"✅ Found weight: {weight_file.name} ({self._format_size(size)})"
                )
        ...
        try:
            ...
            if weight_name not in self.models:
                ...
                if self.device == "cuda":
                    ...
                    model.model.eval()
                    # Enable TensorRT if available (faster inference)
                    try:
                        import torch
    ...
    def detect_in_frame(
        ...
        """Detect logos in a video frame - optimized for GPU"""
        if not self.is_loaded():
            raise RuntimeError("Model not loaded")

        # Run detection with device optimization
        results = self.current_model(
            frame,
            save=False,
            conf=confidence_threshold,
            device=self.device,
            verbose=False,  # Reduce logging for faster inference
            half=(self.device == "cuda"),  # Use FP16 on CUDA for 2x speed boost
            imgsz=640,  # Standard input size for optimal speed
        )
